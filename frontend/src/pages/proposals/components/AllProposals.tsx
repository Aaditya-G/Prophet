...
import { convertBackendProposal, getAIAnalysisForProposal } from '../../../utils/proposalUtils'
import { ProposalDetails } from './ProposalDetails'
import { MarkdownRenderer } from '../../../components/MarkdownRenderer'
import type { SortOption, FilterOption, BackendProposal } from '../types'
...
export const AllProposals = ({ searchTerm }: AllProposalsProps) => {
  const [sortBy, setSortBy] = useState<SortOption>('newest')
  const [filterBy, setFilterBy] = useState<FilterOption>('all')
  const [selectedProposal, setSelectedProposal] = useState<BackendProposal | null>(null)
  const { allProposals, aiAnalysis, loading, error } = useProposals(10)

  // Convert backend proposals to frontend format and filter by search term
  const filteredProposals = allProposals
    .map(convertBackendProposal)
    .filter(proposal => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'active':
        return { backgroundColor: 'rgba(199, 125, 255, 0.2)', color: '#c77dff' }
      case 'passed':
        return { backgroundColor: 'rgba(34, 197, 94, 0.2)', color: '#22c55e' }
      case 'failed':
        return { backgroundColor: 'rgba(255, 255, 255, 0.2)', color: '#ffffff' }
      default:
        return { backgroundColor: 'rgba(107, 114, 128, 0.2)', color: '#6b7280' }
    }
  }
  ...
  const handleCreateProposal = () => {
    // Here you would typically open a modal or navigate to a create proposal page
    ...
    <div className="rounded-3xl p-6" style={{ backgroundColor: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(197, 255, 74, 0.2)' }}>
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-gray-400 text-xs">{proposal.votes} votes</span>
                  </div>
                </div>
      ...
      {selectedProposal && (
        <ProposalDetails
          ...
          aiAnalysis={getAIAnalysisForProposal(selectedProposal.id, aiAnalysis)}
